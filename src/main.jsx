...
const visitImageBucket = 'visit-images';
        ...
        <div className="visit-grid">
          {!loaded ? (
            ...
            <button key={visit.id} className="visit-card" onClick={() => setSelectedVisit(visit)}>
              {Array.isArray(visit.photos) && visit.photos[0]?.url && (
                <img className="visit-card-photo" src={visit.photos[0].url} alt={`${visit.apartment_name} 임장 사진`} />
              )}
...
function VisitEditorPage({ session, visit, onClose, onSubmit }) {
  ...
  const visitPhotoRef = useRef(null);
  ...
  async function addVisitPhotos(event) {
    const files = Array.from(event.target.files || []);
    if (!files.length) return;
    if (!session?.user?.id) {
      setMessage('사진을 올리려면 먼저 로그인되어 있어야 합니다.');
      event.target.value = '';
      return;
    }
    ...
    for (const file of files) {
      const extension = file.name.includes('.') ? file.name.split('.').pop() : 'jpg';
      const path = `${session.user.id}/${Date.now()}-${crypto.randomUUID()}-${safeFileName(file.name || `visit.${extension}`)}`;
      const { error } = await supabase.storage.from(visitImageBucket).upload(path, file, {
        cacheControl: '31536000',
        upsert: false,
      });

      if (error) {
        setMessage(`사진 업로드 실패: ${error.message}. Supabase Storage SQL을 실행했는지 확인해주세요.`);
        break;
      }

      const { data } = supabase.storage.from(visitImageBucket).getPublicUrl(path);
      uploadedPhotos.push({ url: data.publicUrl, name: file.name, path });
    }
  ...
  function removeVisitPhoto(photoUrl) {
    setDraft((current) => ({
      ...current,
      photos: (current.photos || []).filter((photo) => photo.url !== photoUrl),
    }));
  }
  ...
  return (
    <section className="visit-editor-page">
      ...
      <form className="visit-editor-form" onSubmit={submit}>
        ...
        <section className="visit-photo-editor">
          <div>
            <p className="section-kicker">Photos</p>
            <h3>임장 사진</h3>
            <span>현장에서 찍은 외관, 동선, 주차장, 주변 상권 사진을 같이 남겨두세요.</span>
          </div>
          <button type="button" className="secondary-button" onClick={() => visitPhotoRef.current?.click()} disabled={uploadingPhoto}>
            {uploadingPhoto ? <Loader2 className="spin" size={17} /> : <ImagePlus size={17} />}
            사진 추가
          </button>
          <input ref={visitPhotoRef} type="file" accept="image/*" multiple onChange={addVisitPhotos} hidden />
          {(draft.photos || []).length > 0 ? (
            <div className="visit-photo-grid">
              {draft.photos.map((photo) => (
                <figure key={photo.url}>
                  <img src={photo.url} alt={photo.name || '임장 사진'} />
                  <figcaption>
                    <span>{photo.name || '임장 사진'}</span>
                    <button type="button" onClick={() => removeVisitPhoto(photo.url)}>
                      <Trash2 size={14} />
                    </button>
                  </figcaption>
                </figure>
      ...
      <section className="visit-detail-layout">
        ...
        <article className="visit-detail-content">
          ...
          {(normalized.photos || []).length > 0 && (
            <section className="visit-detail-section visit-photo-section">
              <h3>임장 사진</h3>
              <div className="visit-detail-photos">
                {normalized.photos.map((photo) => (
                  <a key={photo.url} href={photo.url} target="_blank" rel="noreferrer">
                    <img src={photo.url} alt={photo.name || '임장 사진'} />
                    <span>{photo.name || '사진 보기'}</span>
                  </a>
                ))}
              </div>
            </section>
